...
class ScrumTimer: ObservableObject {
    ...
    private var lengthInSeconds: Int { length * 60 }
    private var secondsPerSpeaker: Int {
        (length * 60) / speakers.count
    }
    ...
    /**
     ...
     - Parameters:
        - length: The meeting length.
        -  attendees: A list of attendees for the meeting.
     */
    init(length: Int = 0, attendees: [DailyScrum.Attendee] = []) {
        self.length = length
     ...
     */
    func reset(length: Int, attendees: [DailyScrum.Attendee]) {
        self.length = length
        self.speakers = attendees.speakers
        secondsRemaining = lengthInSeconds
        activeSpeaker = speakerText
...
extension DailyScrum {
    ...
    var timer: ScrumTimer {
        ScrumTimer(length: length, attendees: attendees)
    }
}
